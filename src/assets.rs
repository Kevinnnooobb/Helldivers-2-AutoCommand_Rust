...
// plan7 §4.2 允许的偏离（唯一一处）：H2AC 自有的 107 个图标占用了
// `assets/icons/`，参考图标被收在 `assets/reference/icons/`，故此处改写嵌入路径。
#[derive(Embed)]
#[folder = "assets/reference/icons/"]
struct IconAssets;

pub fn default_icon_manifest() -> JsonAsset {
    JsonAsset {
        name: "assets/reference/icons/manifest.json",
        bytes: include_bytes!("../assets/reference/icons/manifest.json"),
    }
}
