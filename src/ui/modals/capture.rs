...
pub fn render_capture_modal(app: &mut H2ACApp, ctx: &Context, m: &UiMetrics) {
        ...
        let _just = key_capture_modal(ctx, "capture", &title, m, &mut captured, |ui, key_name| {
            ui.horizontal(|ui| {
                if hud_button(ui, "确 认", Vec2::new(90.0, 28.0), m, GOLD, false).clicked() {
                    ...
                    if !app_ref.model.config.loadout_sync_hotkey.is_empty()
                        && key_name == app_ref.model.config.loadout_sync_hotkey
                    {
                        app_ref.log(
                            LogKind::Warn,
                            "该键与「自动装配」热键相同，将优先触发自动装配",
                        );
                    }
    ...
    if app.capture.capturing_loadout_sync {
        let title = "自动装配 — 设置全局快捷键";
        let mut captured = std::mem::take(&mut app.capture.captured);
        let app_ref = &mut *app;
        let _just = key_capture_modal(
            ctx,
            "loadout_sync_capture",
            title,
            m,
            &mut captured,
            |ui, key_name| {
                ui.horizontal(|ui| {
                    if hud_button(ui, "确 认", Vec2::new(90.0, 28.0), m, GOLD, false).clicked() {
                        app_ref.model.config.loadout_sync_hotkey = key_name.to_string();
                        config::save_config(&app_ref.model.config);
                        // 热键立即生效，无需重启
                        ...
                        app_ref.log(LogKind::Info, format!("自动装配快捷键: {}", key_name));
                        for conflict in app_ref.model.config.hotkey_conflicts() {
                            app_ref.log(LogKind::Warn, format!("快捷键冲突：{conflict}"));
                        }
                        app_ref.capture.capturing_loadout_sync = false;
                    }
                    if hud_button(ui, "取 消", Vec2::new(90.0, 28.0), m, TEXT_SUB, false).clicked()
                    {
                        app_ref.capture.capturing_loadout_sync = false;
