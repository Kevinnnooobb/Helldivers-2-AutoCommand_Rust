...
pub fn render_detail(app: &mut H2ACApp, ui: &mut Ui, rect: Rect) {
    ...
    hud_panel(&mut region, rect.size(), LINE, |ui| {
        let Some(idx) = app.model.detail_slot.filter(|&i| app.slot_filled(i)) else {
            ui.centered_and_justified(|ui| {
                ui.label(egui::RichText::new("— 点选槽位以查看详情 —").font(hud(13.0)).color(TEXT_DIM));
            });
            return;
        };
        ...
        let text_area = Rect::from_min_size(Pos2::new(icon_area.right(), content.top()), Vec2::new(190.0, h));
        let btn_area = Rect::from_min_size(Pos2::new(content.right() - 88.0, content.top()), Vec2::new(88.0, h));
        ...
        text_ui.vertical(|ui| {
            ui.add_space(6.0);
            ui.label(egui::RichText::new(&name).font(hud_b(22.0)).color(TEXT));

            // ── 分类 chip + 下拉修改 ──
            ui.horizontal(|ui| {
                ui.label(egui::RichText::new(format!("{} ·", model_name)).font(hud(12.0)).color(TEXT_SUB));
                let mut cat_sel = eff_cat.clone();
                egui::ComboBox::from_id_salt("detail_cat")
                    .width(140.0)
                    .selected_text(egui::RichText::new(&cat_sel).font(hud(12.0)).color(accent))
                    .show_ui(ui, |ui| {
                        let cats = app.lib_categories();
                        for cat in &cats {
                            if ui.selectable_label(false, egui::RichText::new(cat).font(hud(12.0))).clicked() {
                                cat_sel = cat.clone();
                            }
                        }
                        if ui.button(egui::RichText::new("✚ 新建分类").font(hud(12.0))).clicked() {
                            cat_sel = String::new();
                        }
                    });
                if cat_sel != eff_cat && !cat_sel.is_empty() {
                    ...
                    ui.add(egui::TextEdit::singleline(&mut new_cat).hint_text("输入新分类名…").font(hud(12.0)).desired_width(110.0));
                ...
                if eff_cat != def_cat {
                    ui.label(egui::RichText::new("✎").font(hud(9.0)).color(GOLD_DIM));
                }
            });
        ...
        btn_ui.vertical_centered(|ui| {
            ui.add_space(6.0);
            if hud_button(ui, "执 行", Vec2::new(88.0, 34.0), GOLD, false).clicked() { app.execute_slot(idx); }
            ui.add_space(4.0);
            ...
            if eff_cat != def_cat {
                ui.add_space(4.0);
                if hud_button(ui, "重置分类", Vec2::new(88.0, 22.0), TEXT_SUB, false).clicked() {
        ...
        let aw = arrow_strip_w(&cmd, 18.0, 6.0);
