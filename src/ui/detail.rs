use crate::stratagems::{dir_to_arrow, STRATAGEMS};
use crate::theme::*;
use crate::widgets::*;
use crate::H2ACApp;
use crate::LogKind;
...
pub fn render_detail(app: &mut H2ACApp, ui: &mut Ui, rect: Rect, m: &UiMetrics) {
    ...
    hud_panel(&mut region, rect.size(), m, LINE, |ui| {
        ...
        let (name, icon_key, cmd, def_cat, model_name): (
            String,
            String,
            Vec<&str>,
            String,
        ...
        let text_area = Rect::from_min_size(
            ...
            Vec2::new(m.detail_text_area_w(), h),
        );
        let btn_area = Rect::from_min_size(
            Pos2::new(content.right() - m.detail_btn_area_w(), content.top()),
            Vec2::new(m.detail_btn_area_w(), h),
        );
            ...
            ui.horizontal(|ui| {
                ...
                egui::ComboBox::from_id_salt("detail_cat")
                    ...
                    .show_ui(ui, |ui| {
                        ...
                        for cat in &cats {
                            if ui
                                .selectable_label(false, egui::RichText::new(cat).font(m.hud(12.0)))
                                .clicked()
                            {
                                cat_sel = cat.clone();
                            }
                        ...
                        if ui
                            .button(egui::RichText::new("新建分类").font(m.hud(12.0)))
                            .clicked()
                        {
                            cat_sel = String::new();
                        }
                ...
                if cat_sel != orig_cat && !cat_sel.is_empty() {
                    ...
                    ui.add(
                        egui::TextEdit::singleline(&mut new_cat)
                            .hint_text("输入新分类名…")
                            .font(m.hud(12.0))
                            .desired_width(110.0),
                    );
        ...
        btn_ui.vertical_centered(|ui| {
            ui.add_space(6.0);
            if hud_button(ui, "执 行", Vec2::new(88.0, 34.0), m, GOLD, false).clicked() {
