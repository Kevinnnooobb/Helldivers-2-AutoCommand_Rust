...
use crate::theme::*;
use eframe::egui::{
    self, Align2, Color32, Context, CornerRadius, Key, Painter, Pos2, Rect, Response, Sense, Shape,
    Stroke, Ui, Vec2,
};
...
pub fn paint_glyph(p: &Painter, r: Rect, g: Glyph, color: Color32) {
    ...
    match g {
        Glyph::Close => {
            let d = u * 0.55;
            p.add(Shape::line(
                vec![Pos2::new(c.x - d, c.y - d), Pos2::new(c.x + d, c.y + d)],
                s,
            ));
            p.add(Shape::line(
                vec![Pos2::new(c.x + d, c.y - d), Pos2::new(c.x - d, c.y + d)],
        ...
        Glyph::Trash => {
            ...
            let top = c.y - h * 0.55;
            p.add(Shape::line(
                vec![Pos2::new(c.x - w, top), Pos2::new(c.x + w, top)],
                s,
            ));
            p.add(Shape::line(
                vec![
        ...
        Glyph::Restore => {
            ...
            let off = u * 0.3;
            let back =
                Rect::from_center_size(Pos2::new(c.x + off, c.y - off), Vec2::splat(d * 2.0));
            let front =
                Rect::from_center_size(Pos2::new(c.x - off, c.y + off), Vec2::splat(d * 2.0));
...
pub fn glyph_button(ui: &mut Ui, glyph: Glyph, size: f32, tip: &str) -> Response {
    ...
    if hovered {
        paint_chamfer(&p, rect.shrink(1.0), size * 0.13, BG_HOVER, Stroke::NONE);
    }
...
pub fn hud_button(
    ...
    paint_chamfer(
        ...
        6.0 * m.scale,
        fill,
        Stroke::new(1.0, base.gamma_multiply(0.7)),
    );
    if hovered {
        corner_brackets(&p, rect.shrink(2.0 * m.scale), 4.0 * m.scale, base);
    }
    p.text(
        rect.center(),
...
pub fn key_capture_modal(
    ...
    if mods.ctrl && !prev_ctrl && !just_captured {
        *captured_value = "ctrl".to_string();
        just_captured = true;
    }
    if mods.alt && !prev_alt && !just_captured {
        *captured_value = "alt".to_string();
        just_captured = true;
    }
    if mods.shift && !prev_shift && !just_captured {
    ...
    egui::Area::new(egui::Id::new(area_id))
        ...
        .show(ctx, |ui| {
            hud_panel(
                ui,
                Vec2::new(m.modal_keycap_w(), m.modal_keycap_h()),
                m,
                GOLD_DIM,
                |ui| {
                        ...
                        if captured_value.is_empty() {
                            paint_glyph(
                                ...
                                Rect::from_center_size(
                                    Pos2::new(
                                        ui.cursor().center().x,
                                        ui.cursor().min.y + 14.0 * m.scale,
                                    ),
                                    Vec2::splat(28.0 * m.scale),
                                ),
                                Glyph::Keyboard,
                                TEXT_SUB,
                            );
