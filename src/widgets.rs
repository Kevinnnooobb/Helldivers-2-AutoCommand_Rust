...
/// 低透明度扫描线背景
pub fn scanlines(p: &Painter, r: Rect, m: &UiMetrics) {
    let s = m.scale;
    let line = Color32::from_rgba_unmultiplied(0xFF, 0xFF, 0xFF, 2);
    let mut y = r.top() + 2.0 * s;
    while y < r.bottom() {
...
pub fn paint_arrow(p: &Painter, c: Pos2, dir: &str, size: f32, color: Color32) {
    let h = size / 2.0;
    let pts = if dir == UP {
        vec![Pos2::new(c.x, c.y - h), Pos2::new(c.x + h, c.y + h), Pos2::new(c.x - h, c.y + h)]
    } else if dir == DOWN {
        vec![Pos2::new(c.x, c.y + h), Pos2::new(c.x + h, c.y - h), Pos2::new(c.x - h, c.y - h)]
    } else if dir == LEFT {
        vec![Pos2::new(c.x - h, c.y), Pos2::new(c.x + h, c.y - h), Pos2::new(c.x + h, c.y + h)]
    } else if dir == RIGHT {
        vec![Pos2::new(c.x + h, c.y), Pos2::new(c.x - h, c.y - h), Pos2::new(c.x - h, c.y + h)]
    } else {
        return;
...
pub fn arrow_strip(p: &Painter, origin: Pos2, cmd: &[&str], size: f32, gap: f32, color: Color32) -> f32 {
    ...
    if cmd.is_empty() { 0.0 } else { step * cmd.len() as f32 - gap }
}

pub fn arrow_strip_w(cmd: &[&str], size: f32, gap: f32) -> f32 {
    if cmd.is_empty() { 0.0 } else { (size + gap) * cmd.len() as f32 - gap }
}
...
pub enum Glyph { Close, Minimize, Gear, Trash, Save, Compact, Restore, Keyboard, Play, Search }
...
pub fn paint_glyph(p: &Painter, r: Rect, g: Glyph, color: Color32) {
    ...
    match g {
        Glyph::Close => {
            let d = u * 0.55;
            p.add(Shape::line(vec![Pos2::new(c.x - d, c.y - d), Pos2::new(c.x + d, c.y + d)], s));
            p.add(Shape::line(vec![Pos2::new(c.x + d, c.y - d), Pos2::new(c.x - d, c.y + d)], s));
        }
        Glyph::Minimize => {
            let d = u * 0.55;
            p.add(Shape::line(vec![Pos2::new(c.x - d, c.y), Pos2::new(c.x + d, c.y)], s));
        }
        Glyph::Gear => {
            ...
            for i in 0..8 {
                ...
                p.add(Shape::line(vec![Pos2::new(c.x + sx * u * 0.58, c.y + sy * u * 0.58), Pos2::new(c.x + sx * u * 0.85, c.y + sy * u * 0.85)], s));
            }
        }
        Glyph::Trash => {
            let (w, h) = (u * 0.62, u * 0.62);
            let top = c.y - h * 0.55;
            p.add(Shape::line(vec![Pos2::new(c.x - w, top), Pos2::new(c.x + w, top)], s));
            p.add(Shape::line(vec![Pos2::new(c.x - w * 0.35, top), Pos2::new(c.x - w * 0.35, top - h * 0.3), Pos2::new(c.x + w * 0.35, top - h * 0.3), Pos2::new(c.x + w * 0.35, top)], s));
            p.add(Shape::line(vec![Pos2::new(c.x - w * 0.7, top), Pos2::new(c.x - w * 0.5, c.y + h * 0.7), Pos2::new(c.x + w * 0.5, c.y + h * 0.7), Pos2::new(c.x + w * 0.7, top)], s));
        }
        Glyph::Save => {
            let d = u * 0.7;
            let rr = Rect::from_center_size(c, Vec2::splat(d * 2.0));
            p.rect_stroke(rr, CornerRadius::ZERO, s, egui::StrokeKind::Inside);
            let inner = Rect::from_min_size(Pos2::new(rr.left() + d * 0.35, rr.top()), Vec2::new(d * 0.7, d * 0.55));
            p.rect_stroke(inner, CornerRadius::ZERO, s, egui::StrokeKind::Inside);
            p.hline(rr.left() + d * 0.4..=rr.right() - d * 0.4, rr.bottom() - d * 0.45, s);
        }
        Glyph::Compact => {
            let d = u * 0.75;
            let a = u * 0.35;
            for (cx, cy, dx, dy) in [(c.x - d, c.y - d, 1.0f32, 1.0f32), (c.x + d, c.y - d, -1.0, 1.0), (c.x + d, c.y + d, -1.0, -1.0), (c.x - d, c.y + d, 1.0, -1.0)] {
                p.add(Shape::line(vec![Pos2::new(cx + dx * a, cy), Pos2::new(cx, cy), Pos2::new(cx, cy + dy * a)], s));
            }
        }
        Glyph::Restore => {
            let d = u * 0.45;
            let off = u * 0.3;
            let back = Rect::from_center_size(Pos2::new(c.x + off, c.y - off), Vec2::splat(d * 2.0));
            let front = Rect::from_center_size(Pos2::new(c.x - off, c.y + off), Vec2::splat(d * 2.0));
        ...
        Glyph::Keyboard => {
            ...
            for i in -1..=1 { p.circle_filled(Pos2::new(c.x + i as f32 * u * 0.4, c.y - u * 0.12), 1.2, color); }
            p.hline(c.x - u * 0.4..=c.x + u * 0.4, c.y + u * 0.22, Stroke::new(1.2, color));
        }
        Glyph::Play => {
            let d = u * 0.6;
            p.add(Shape::convex_polygon(vec![Pos2::new(c.x - d * 0.6, c.y - d), Pos2::new(c.x + d, c.y), Pos2::new(c.x - d * 0.6, c.y + d)], color, Stroke::NONE));
        }
        Glyph::Search => {
            let cr = u * 0.45;
            let cc = Pos2::new(c.x - u * 0.15, c.y - u * 0.15);
            p.circle_stroke(cc, cr, s);
            p.add(Shape::line(vec![Pos2::new(cc.x + cr * 0.75, cc.y + cr * 0.75), Pos2::new(c.x + u * 0.6, c.y + u * 0.6)], s));
        }
    }
}
...
pub fn glyph_button(ui: &mut Ui, glyph: Glyph, size: f32, tip: &str) -> Response {
    ...
    if hovered { paint_chamfer(&p, rect.shrink(1.0), size * 0.13, BG_HOVER, Stroke::NONE); }
...
pub fn hud_button(ui: &mut Ui, label: &str, size: Vec2, m: &UiMetrics, accent: Color32, danger: bool) -> Response {
    let (resp, p) = ui.allocate_painter(size, Sense::click());
    let rect = resp.rect;
    let hovered = resp.hovered();
    let base = if danger { DANGER } else { accent };
    let fill = if hovered { base.gamma_multiply(0.22) } else { base.gamma_multiply(0.08) };
    paint_chamfer(&p, rect, 6.0 * m.scale, fill, Stroke::new(1.0, base.gamma_multiply(0.7)));
    if hovered { corner_brackets(&p, rect.shrink(2.0 * m.scale), 4.0 * m.scale, base); }
    p.text(rect.center(), egui::Align2::CENTER_CENTER, label, m.hud(13.0), if hovered { TEXT } else { base });
    resp
}

// ─── 固定尺寸切角面板（内含子 UI） ───

pub fn hud_panel(ui: &mut Ui, size: Vec2, m: &UiMetrics, border: Color32, add: impl FnOnce(&mut Ui)) -> Rect {
    let rect = Rect::from_min_size(ui.cursor().min, size);
    ui.advance_cursor_after_rect(rect);
    let c = m.chamfer();
    paint_chamfer(ui.painter(), rect, c, BG_PANEL, Stroke::new(1.0, border));
    corner_brackets(ui.painter(), rect.shrink(3.0 * m.scale), 7.0 * m.scale, GOLD_DIM);
    let mut child = ui.new_child(
        egui::UiBuilder::new()
            .max_rect(rect.shrink2(Vec2::new(14.0 * m.scale, 10.0 * m.scale)))
            .layout(*ui.layout()),
    );
    add(&mut child);
    rect
...
pub fn key_capture_modal(
    ctx: &Context, area_id: &str, title: &str, m: &UiMetrics,
    captured_value: &mut String, buttons: impl FnOnce(&mut Ui, &str),
) -> bool {
    ...
    egui::Area::new(egui::Id::new(area_id))
        ...
        .show(ctx, |ui| {
            hud_panel(ui, Vec2::new(m.modal_keycap_w(), m.modal_keycap_h()), m, GOLD_DIM, |ui| {
                ui.vertical_centered(|ui| {
                    ui.add_space(8.0 * m.scale);
                    ui.label(egui::RichText::new(title).font(m.hud_b(15.0)).color(GOLD));
                    ui.add_space(10.0 * m.scale);
                    if captured_value.is_empty() {
                        paint_glyph(ui.painter(), Rect::from_center_size(
                            Pos2::new(ui.cursor().center().x, ui.cursor().min.y + 14.0 * m.scale), Vec2::splat(28.0 * m.scale)),
                            Glyph::Keyboard, TEXT_SUB);
                        ui.add_space(30.0 * m.scale);
                        ui.label(egui::RichText::new("按下目标按键…").font(m.hud(13.0)).color(TEXT_SUB));
                    } else {
                        let key_name = captured_value.clone();
                        ui.label(egui::RichText::new(key_name.to_uppercase()).font(m.hud_b(24.0)).color(OK));
                        ui.add_space(10.0 * m.scale);
                        buttons(ui, &key_name);
                    }
                });
