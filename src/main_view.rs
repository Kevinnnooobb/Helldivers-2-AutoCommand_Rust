...
use crate::ui::modals::{
    render_capture_modal, render_context_menu, render_library_context_menu, render_settings_modal,
    render_stratagem_settings,
};
...
pub fn show_main(app: &mut H2ACApp, ctx: &Context) {
    ...
    egui::CentralPanel::default()
        ...
        .show(ctx, |ui| {
            ...
            let bottom = Rect::from_min_max(
                Pos2::new(full.left(), full.bottom() - m.bottombar_h()),
                full.max,
            );
            ...
            let right = Rect::from_min_max(
                Pos2::new(left.right() + m.panel_gap(), inner.top()),
                inner.max,
            );
    ...
    if ctx.input(|i| i.key_pressed(Key::Escape)) && app.model.armed.is_some() {
