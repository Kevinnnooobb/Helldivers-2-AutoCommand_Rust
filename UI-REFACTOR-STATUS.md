...
> - ✅ **Release 构建通过**：25.7s，exe 4.75MB，20s 稳定运行，PrintWindow 截屏验证渲染正确
>   （分类色底条、热键角标、静音状态灯均正常）
> - ⚠️ `hud_panel` 的巨型 `Sense::hover()` 分配已移除（改为 cursor 定位 + advance），
>   分类栏 bug 的诱因之一已消除，但**该 bug 未再复验**（用户决定跳过，见第 5 节）
> - ⏭️ 分类栏点击 bug **用户决定跳过**，证据保留在第 5 节供日后排查
